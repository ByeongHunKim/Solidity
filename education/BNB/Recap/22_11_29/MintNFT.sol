...
contract MintNFT is ERC721Enumerable, Ownable {
  ...
  constuctor(string memory _metadataURI, string memory _notRevealedURI, uint _totalNFT) ERC721("BNB,Challenger", "BC"){
    metadataURI = _metadataURI;
    notRevealedURI = _notRevealedURI;
    totalNFT = _totalNFT;
  }

  function setRevealTime(uint _ tokenId, uint _revealTime) public onlyOwner {
  ...
  function tokenURI(uint _tokenId) public override view returns (string memory) {
    if(isRevealed == false) return notRevealdURI;
